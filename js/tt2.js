...
$.ajax({
	url: 'js/cards.json',			// AJAX call to get GitHub profile info
	type: 'GET',
	dataType: 'json',
...
(function($) {
	...
	$('.pick-hand button').on('click', function(e) {
		...
		if ($('.pick-hand .card-list input:checked').length !== maxCards) {
			...
			$.each($('.pick-hand .card-list input:checked'), function(index, value) {
				...
				var card = $(cardList).attr(name).slice(0);
				p0hand.push(card);
			});
			...
			for (var i = 0; i < maxCards; i++) {
				var index = randIndicies[i];
				var card = cardList[Object.keys(cardList)[index]].slice(0);
				p1hand.push(card);
				p0hand[i][0] = 0;
				p1hand[i][0] = 1;
				placeCard($('#p0-deck .hand').eq(i), p0hand[i], 0);
				if (!show) {
					$('#p1-deck .hand').eq(i).html('<img class="card-img" src="img/TTback.jpg"/>');
				}
				else {
					placeCard($('#p1-deck .hand').eq(i), p1hand[i], 1);
				}
			}
	...
	function playCard(element) {
		if (onDeck == null) {
			...
			var p = turn % 2;
			placeCard(id + ' .front', onDeck, turn % 2);
			placeCard(id + ' .back', onDeck, (turn + 1) % 2);
			$(id).flip({
				trigger: 'manual'
			});
	...
	function randomAI() {
		var selectTO = setTimeout(function() {
			selectCard($('#p1-deck .hand').eq(Math.floor(Math.random() * 5)).children());
		}, 800);
		var playTO = setTimeout(function() {
			playCard('#card' + Math.floor(Math.random() * 9));
		}, 1600);
				...
				if (board[i][0] === 0) {
					if (i % 3 != 0 && board[i - 1] == null) {
						for (var j = 0; j < maxCards; j++) {
							if (p1hand[j] != null) {
								if (p1hand[j][3] > board[i][5]) {
									current = 1;
									if (board[i - 4] != (null || undefined)) {
										if (board[i - 4][0] === 0 && p1hand[j][2] > board[i - 4][4]) {
											current++;
										}
									}
									if (board[i + 2] != (null || undefined)) {
										if (board[i + 2][0] === 0 && p1hand[j][4] > board[i + 2][2]) {
											current++;
										}
									}
									if ((i - 2) % 3 == 0 && board[i - 2] != (null || undefined)) {
										if (board[i - 2][0] === 0 && p1hand[j][5] > board[i - 2][3]) {
											current++;
										}
									}
								}
								...
								if (current == max) {
									if (max == 0) {
										...
										var currentSum = p1hand[cardToPlay][2] + p1hand[cardToPlay][3] + p1hand[cardToPlay][4] + p1hand[cardToPlay][5];
										var newSum = p1hand[j][2] + p1hand[j][3] + p1hand[j][4] + p1hand[j][5];
					...
					if ((i + 1) % 3 != 0 && board[i + 1] == null) {
						for (var j = 0; j < maxCards; j++) {
							if (p1hand[j] != null) {
								if (p1hand[j][5] > board[i][3]) {
									current = 1;
									if (board[i - 2] != (null || undefined)) {
										if (board[i - 2][0] === 0 && p1hand[j][2] > board[i - 2][4]) {
											current++;
										}
									}
									if (board[i + 4] != (null || undefined)) {
										if (board[i + 4][0] === 0 && p1hand[j][4] > board[i + 4][2]) {
											current++;
										}
									}
									if (i % 3 == 0 && board[i + 2] != (null || undefined)) {
										if (board[i + 2][0] === 0 && p1hand[j][3] > board[i + 2][5]) {
											current++;
										}
									}
								}
					...
					if (i > 2 && board[i - 3] == null) {
						for (var j = 0; j < maxCards; j++) {
							if (p1hand[j] != null) {
								if (p1hand[j][4] > board[i][2]) {
									current = 1;
									if (board[i - 6] != (null || undefined)) {
										if (board[i - 6][0] === 0 && p1hand[j][2] > board[i - 6][4]) {
											current++;
										}
									}
									if ((i - 1) % 3 != 0 && board[i - 4] != (null || undefined)) {
										if (board[i - 4][0] === 0 && p1hand[j][5] > board[i - 4][3]) {
											current++;
										}
									}
					...
					if (i < 6 && board[i + 3] == null) {
						for (var j = 0; j < maxCards; j++) {
							if (p1hand[j] != null) {
								if (p1hand[j][2] > board[i][4]) {
									current = 1;
									if (board[i + 6] != (null || undefined)) {
										if (board[i + 6][0] === 0 && p1hand[j][4] > board[i + 6][2]) {
											current++;
										}
									}
									if ((i - 1) % 3 != 0 && board[i + 2] != (null || undefined)) {
									...
									if ((i + 1) % 3 != 0 && board[i + 4] != (null || undefined)) {
										if (board[i + 4][0] === 0 && p1hand[j][3] > board[i + 4][5]) {
											current++;
										}
									}
								}
	...
	function placeCard(place, card, owner) {
		$(place).html('<img class="card-img" src="cards/' + ((owner === 0) ? "" : "r") +
			card[1] + '.jpg"/>');
	}
			...
			for (var i = 0; i < 5; i++) {
				if (p1hand[i] != null) {
					$('#p1-deck .hand').eq(i).fadeOut("fast", function() {
						...
						placeCard($('#p1-deck .hand').eq(n), p1hand[n], 1);
